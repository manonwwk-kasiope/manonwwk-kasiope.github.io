...
// rapporté, ainsi que le signal des portails : la mesure n'est concluante que si le signal DOMINE le
// bruit (p10 du signal >= 3 x p95 du bruit), sinon le test sort en code 2.
...
try {
  ...
  if (!band) { m.pourquoi = 'canvas #game introuvable'; code = 2; }
    ...
    // signal réellement mesuré dans la bande pendant les fenêtres de portail :
    // c'est lui qu'il faut comparer au bruit de fond pour dire si la mesure sépare.
    const pxs = R.portals.map(r => r.px).sort((a, b) => a - b);
    const sq = p => pxs.length ? pxs[Math.min(pxs.length - 1, Math.floor(pxs.length * p))] : 0;
    const sig = { n: pxs.length, p10: sq(0.1), p50: sq(0.5), p90: sq(0.9), max: pxs.length ? pxs[pxs.length - 1] : 0 };
    m.mesure = { apparitions: n, ignoreesAvantJournal: horsPortee, portauxJournalises: R.portals.length, entreesJournal: R.logLen,
      ...
      secondesDeJeu: Math.round(R.tGame / 1000), phaseFinale: R.phase, exemples: misses, signalPortails: sig };
    ...
    // garde-fou de séparation : la mesure ne prouve quelque chose que si le signal
    // des portails domine franchement le bruit de fond de la bande. Un seuil absolu
    // sur le seul bruit (100 px, la valeur même du critère) refusait la mesure pour
    // un bruit p95 = 100 alors que le signal était dix fois plus haut ; on exige
    // maintenant un rapport : p10 du signal >= 3 x p95 du bruit.
    else if (sig.p10 < 3 * Math.max(1, R.noise.p95)) {
      m.pourquoi = 'séparation insuffisante : p10 du signal ' + sig.p10 + ' px contre bruit de fond p95 ' +
                   R.noise.p95 + ' px (rapport exigé 3), mesure non concluante';
      code = 2;
    }
